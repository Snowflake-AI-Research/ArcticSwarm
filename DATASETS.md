...
bash scripts/fetch_datasets.sh --skip-evo # skip EvoBrowseComp (which requires HuggingFace)
```
...
| `evobrowsecomp_v1.csv` | 400 | HuggingFace `Krystalan/EvoBrowseComp` (English split) |
...
### EvoBrowseComp
- Source: HuggingFace [`Krystalan/EvoBrowseComp`](https://huggingface.co/datasets/Krystalan/EvoBrowseComp)
  (800 bundled examples; we keep the 400 English questions, tagged `EVOBROWSECOMP_V1`).
  XOR one-time-pad encrypted; the loader decrypts at runtime.
- License: see the HuggingFace dataset card.
- Cite: Wang, Wang, Huang, Zeng, Meng, *EvoBrowseComp: Benchmarking Search Agents on
  Evolving Knowledge* (see the dataset card for the canonical reference).
