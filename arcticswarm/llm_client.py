...
class AnthropicLLMClient(BaseLLMClient):
    ...
    # max_tokens guards
    # ------------------------------------------------------------------
    # Empirical: claude-sonnet-4-6 on Snowhouse preprod rejects requests with
    # ``max_tokens`` above ~128k with "The maximum tokens you requested
    # exceeds the model limit". Leave a buffer.
    ...
    def _apply_max_tokens_guards(
        ...
        1. **Parameter cap** (always): ``max_tokens`` cannot exceed the model's
           own ``max_tokens`` parameter limit (~128k for sonnet-4-6 on preprod).
           We cap at ``_MAX_TOKENS_PARAM_CAP`` (100k) to leave a safety buffer.
