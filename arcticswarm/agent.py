...
class Agent(ToolExecutionMixin, ContextManagementMixin, FallbackMixin):
    ...
    def __init__(
        ...
        # model-trained refusals.  See
        # docs/analysis-gpt5.4-failure/03_hypothesis_azure_safety_filtering.md.
    ...
    #     85% of ALL_FAILED events on GPT-5.4 runs were "Error code: 400 ...
    #     max tokens of 200000 exceeded" from Sonnet 4 receiving > 200K input
    #     after a GPT-5.4 primary in 1M-context mode.  See
    #     ``docs/analysis-gpt5.4-failure/06_hypothesis_fallback_degradation.md``
    #     Fix A.
    ...
    # rungs of the empty-response fallback ladder fail 96% of the time.
    # Caching the failures saves ~50% of fallback latency per case.
    # See docs/analysis-gpt5.4-failure/12_hypothesis_broken_fallback_chain.md
    # Fix 3.  Process-local (not persisted) — avoids cross-deploy poisoning.
    ...
    def _save_response_chain(self, response: "LLMResponse") -> None:
        ...
        if getattr(self.config, "disable_responses_chaining", False):
            ...
            # See docs/analysis-gpt5.4-failure/04_hypothesis_responses_api_issues.md.
            self._last_response_id = None
            self._msg_checkpoint = 0
            return
        ...
        # See 04_hypothesis_responses_api_issues.md Experiment C.
    ...
    def run_turn(
        ...
        while turns < effective_max_turns:
            ...
            # Execute tools (parallel when all are I/O-bound, else sequential).
