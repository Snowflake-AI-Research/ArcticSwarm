---
name: swarm-orchestration-dm-spawnable
description: >
  Orchestrator workflow for coordinating a team of subagents via task
  board delegation in DM-only mode (no BBS) with agent_spawnable support
  (create_agent, assign_to). Covers the workflow (understand, spawn team,
  delegate, wait, review, report), visualization guidelines, citation
  protocol, and task creation best practices.
---

# Swarm Orchestration Skill (DM-Only, Spawnable)
...
as **"leader"** and will receive broadcast messages from subagents when
they share findings with the team via `send_message to='all'`.

You will see these broadcasts automatically between tool calls. Combined
with the **task completion summaries** from `wait_for_tasks`, you have
visibility into both the final results and the intermediate findings
...
3. **Post tasks to the board**. Use `create_task` to post tasks. Subagents
...
4. **Wait for results**. Use `wait_for_tasks` to block until tasks finish.

5. **Review and reconcile** (MANDATORY). After `wait_for_tasks` returns:
   a. Review the task completion summaries and any broadcast DMs you received.
   ...
   d. Only proceed to step 6 when you are confident the findings are sound.

6. **Verify before reporting** (MANDATORY). Review all task summaries,
   intermediate results, and the original question. Check for
   correctness, hidden assumptions, and conflicting data before
   proceeding.

7. **Prepare and deliver your report**. Call `prepare_report`, then
...
- Subagents communicate via direct messaging among themselves. You will see their findings only through task completion summaries.
