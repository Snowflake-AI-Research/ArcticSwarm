---
name: swarm-orchestration-dynamic-web-noverify
description: >
  Ablation variant of swarm-orchestration-dynamic-web with the orchestrator's
  final-verification / alternative-forcing instructions removed (GATE 3 OFF).
  Board review (auditor + idle review) is retained. Used by the R2 arm of the
  review-gate ablation; do not use for production runs.
---
...
4. **Review** (MANDATORY). Check `read_bbs` especially **#discussion**.
