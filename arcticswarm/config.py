...
class ArcticswarmConfig:
    ...
    # extra orchestrator turn asking for a best-guess. Catches the 14/31
    # wrong cases that never reach Layer 4a because there's no answer to verify.
    enable_empty_answer_recovery: bool = False

    # 0628 answer-retention fix: when True, prepare_report appends a
    ...
    # posts), into the report-unlock message. Counters the ">=31% of wrong cases
    # had the correct answer in the swarm's own findings but not in the final
    ...
    # 0628 selective-delete compaction (user request): before the proactive
    ...
    # 0628 alt-task dispatch fix: the candidate-emergence rival sweep only
    ...
    # 0 tool_uses in ~40% of cases (measured). When True, the hook ALSO calls
    ...
    # 0628 anti-anchoring: append the ANTI_ANCHOR_BLOCK to the browsing agent
    ...
    # scan snippets). Targets the dominant recall-miss failure (17/19 never-found
    # cases anchored on a wrong framing). Default False; qwen-gated via YAML.
    reframe_prompt: bool = False

    # 0629 anti-give-up + canonical-answer report behavior. When True,
