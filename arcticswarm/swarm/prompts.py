...
"""
    ...
    # SWE-bench leaders also benefit from the minimal-fix skill, which makes
    # explicit that "ship a /testbed patch" beats "write a markdown report".
    # ``harbor_runner`` injects this skill into ``orchestrator_skills`` for
    # SWE-bench datasets so it's available via ``load_skill``.
    _is_swebench = dataset in {"swebench-verified", "swebenchpro"}
    if _is_swebench:
        if per_skill_tools:
            skills_blurb += (
                ' Also call `skill-swebench-minimal-fix` for the '
                "SWE-bench patch-landing protocol — your delegated "
                "subagents must actually edit files in `/testbed`, "
                "not produce analysis-only reports."
            )
        else:
            skills_blurb += (
                ' Also call `load_skill("swebench-minimal-fix")` '
                "for the SWE-bench patch-landing protocol — your "
                "delegated subagents must actually edit files in "
                "`/testbed`, not produce analysis-only reports."
            )
...
# 0628 anti-anchoring (qwen-gated via config.reframe_prompt). Appended to the
...
# frame (17/19 never-found cases), with huge search:fetch ratios (~249:13).
ANTI_ANCHOR_BLOCK = """
...
run your role is a **patch reviewer and test validator**, NOT a parallel \
patch author. Your duo partner (**{partner_name}**) is driving the patch \
itself; your job is to critically test it and report findings.

**Today's date is {current_date}.**
{workspace_block}
## First step

Call `load_skill("swebench-auditor-review")` (NOT \
`swebench-minimal-fix` — that skill's "Work directly in /testbed" \
instructions are for the leader). The skill gives you the full \
pull/apply/test/DM workflow plus the exact DM format.

## Your Responsibilities

1. **Pull the leader's current patch** — your "Your Environment" \
block above names the exact `git -C ... diff HEAD` command. Re-pull \
whenever you suspect they have moved on (e.g. after one of their \
tool-call DMs lands).
2. **Apply their patch into your scratch worktree** and then run the \
failing target test plus adjacent tests under `python -m pytest` \
against your worktree. This tells you whether the leader's patch \
actually passes the verifier's test, BEFORE they submit.
3. **Probe edges** — try alternative inputs (empty, large, boundary, \
error paths) and inspect adjacent code paths that share the bug class. \
You MAY make experimental edits in your worktree to test "what if" \
hypotheses; these are scratch and will NOT be harvested into a \
deliverable.
4. **DM the leader with structured findings** via `send_message` in \
this exact format:

```
PATCH: <one-line summary of what the leader did>
STATUS: PASS | FAIL | PARTIAL
FINDINGS:
- <test or edge case>: <result>
- <regression or missed case>: <result>
SUGGESTIONS:
- <concrete change recommendation, e.g. "also handle None in foo.py:42">
```

   Send the first such DM as soon as you have run the target test once. \
Re-send after probing edges. Each DM should be concrete (failing test \
name + assertion, or a missed input + expected vs actual).
5. **Complete your task** — call `complete_task` with a thorough \
summary once you have reported PASS/FAIL on the target test plus at \
least one edge-case probe.

## What you must NOT do

- **Do NOT produce your own competing patch** to the leader's source \
files. Two patches will conflict and confuse the leader; in this run \
the leader is the single source of truth for what lands. Writes to \
the leader's tree are blocked by the filesystem tools regardless.
- **Do NOT just analyze the bug abstractly.** Your unique value is \
CONCRETE test results, not re-deriving the root cause.
