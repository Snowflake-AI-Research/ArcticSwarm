...
_CONTAMINATION_KEYWORDS = (
    "browsecomp",
    "evobrowsecomp",  # superset of "browsecomp" by substring today; explicit so
                      # protection survives a future word-boundary match change.
)
...
class ToolExecutionMixin:
    ...
    def _execute_tools_batch(
        ...
        """Execute a batch of tool calls, parallelizing I/O-bound tools.

        When all requested tools are in ``_PARALLEL_SAFE_TOOLS`` and there are
        at least two calls, the raw ``tool.execute()`` calls run concurrently
        via threads.  Post-processing (contamination filtering, source
        tracking, counter updates) always runs sequentially afterwards to
        avoid shared-state races.
        """
        # Parallel execution disabled for quality investigation.
