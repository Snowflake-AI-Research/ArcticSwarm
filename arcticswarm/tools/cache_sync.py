...
Standalone merge (final/manual): ``snowflake/scripts/sync_cache.py``. See
snowflake/snowflake_specific.md for the Snowflake-cluster paths/bucket.
"""
...
#   search docs:   PK(provider,qkey,url); first wins (IGNORE); ordered by seq.
...
_SEARCH_COLS = ("provider", "qkey", "url", "title", "snippet", "extra", "seq")
...
def should_auto_mirror(config) -> bool:
    ...
    caching is active (a fetch cache path and/or the search cache is on), and
    node-local fast storage (an ancestor of ``cache_local_dir``) actually
    exists. This keeps the mirror automatic on cluster nodes while making
    it a silent no-op on machines without a fast-disk mount.
    """
    if not getattr(config, "cache_local_mirror", True):
        return False
    caching_active = bool((getattr(config, "fetch_cache_path", "") or "").strip()) or (
        getattr(config, "enable_search_cache", False)
        and bool((getattr(config, "search_cache_db", "") or "").strip())
    )
    if not caching_active:
        return False
    local_dir = (getattr(config, "cache_local_dir", "") or "/data-fast/cache").strip()
...
class _Cache:
    """One mirrored cache (fetch or search)."""
...
class CacheMirrorManager:
    ...
    def __init__(self, config, output_dir: str):
        ...
        self.local_dir = (getattr(config, "cache_local_dir", "") or "/data-fast/cache").strip()
    ...
    def setup(self) -> None:
        ...
        search_master = (getattr(self.config, "search_cache_db", "") or "").strip()
        if getattr(self.config, "enable_search_cache", False) and search_master:
            local = os.path.join(self.local_dir, "search_cache.sqlite")
            c = _Cache(search_master, local, table="docs", cols=_SEARCH_COLS,
                       conflict=" ON CONFLICT(provider, qkey, url) DO NOTHING", order_col="seq")
            c.mirror()
            self.config.search_cache_db = local
            self.caches.append(c)
        log.info(
