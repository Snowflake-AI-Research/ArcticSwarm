...
General mechanism: when a cache the run depends on (fetch cache, search cache)
is absent at process start, and an S3 mirror is configured, restore it with a
single ``aws s3 sync`` before opening. Disabled by default — a no-op unless
...
``ARCTICSWARM_CACHE_S3_PREFIX``. See snowflake/snowflake_specific.md for the
Snowflake-cluster bucket/prefix/paths we use.
"""
...
def ensure_cache_restored(
    ...
    *,
    ...
    with _lock:
        ...
        # sync_target itself (it may be a file path, e.g. the search cache).
        try:
