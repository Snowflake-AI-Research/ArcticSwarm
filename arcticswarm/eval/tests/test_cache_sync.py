...
Covers the fetch (entries, longest-wins) and search (docs, first-wins) merge,
the rowid/seq delta watermark, and CacheMirrorManager repointing the config to
the local mirror. No network / flock contention (single process, tmp files).
"""
...
def test_search_docs_first_wins(tmp_path):
    def mk(path, rows):
        c = sqlite3.connect(str(path))
        c.execute("CREATE TABLE docs (provider TEXT, qkey TEXT, url TEXT, title TEXT, snippet TEXT,"
                  " extra TEXT, seq INTEGER NOT NULL, PRIMARY KEY(provider,qkey,url))")
        c.executemany("INSERT INTO docs(provider,qkey,url,title,snippet,extra,seq) VALUES(?,?,?,?,?,?,?)", rows)
        c.commit(); c.close()
    master = tmp_path / "sm.sqlite"; local = tmp_path / "sl.sqlite"
    mk(master, [("brave", "q1", "u1", "t", "s", None, 1)])
    mk(local, [("brave", "q1", "u1", "t2", "s2", None, 1), ("brave", "q1", "u2", "t", "s", None, 2)])
    c = cs._Cache(str(master), str(local), table="docs", cols=cs._SEARCH_COLS,
                  conflict=" ON CONFLICT(provider, qkey, url) DO NOTHING", order_col="seq")
    c.watermark = 0
    c.sync()
    m = sqlite3.connect(str(master))
    rows = dict(((p, k, u), title) for p, k, u, title in m.execute("SELECT provider,qkey,url,title FROM docs"))
    assert rows[("brave", "q1", "u1")] == "t"   # existing kept (first-wins / IGNORE)
    assert ("brave", "q1", "u2") in rows         # new url merged
...
def test_manager_setup_repoints_config(tmp_path):
    ...
    cfg = SimpleNamespace(
        fetch_cache_path=str(master), search_cache_db="", enable_search_cache=False,
...
def test_should_auto_mirror_gating(tmp_path):
    base = dict(cache_local_mirror=True, cache_local_dir=str(tmp_path),
                ...
                enable_search_cache=False, search_cache_db="")
    ...
    # search cache alone (no fetch) still counts as caching active
    assert cs.should_auto_mirror(SimpleNamespace(**{
        **base, "fetch_cache_path": "", "enable_search_cache": True,
        "search_cache_db": str(tmp_path / "s.sqlite")}))
