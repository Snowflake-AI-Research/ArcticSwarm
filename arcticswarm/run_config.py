...
class SwarmConfig:
    ...
    # stale-view race (see /tmp/audit_duo_conflicts.py). Default off for
    ...
    # structurally absent in duo/swebench mode (no ``prepare_report`` barrier). Motivated by the
    # reviewer-diversity finding (both-source cases reach 86.4% on BrowseComp vs
    # 35.3% with no verdict). See ``scripts/analyze_reviewer_diversity.py``.
...
class RunConfig:
    ...
    date_override: str = ""
    prompt_style: str = ""
    # docs/0511 best-of-N alternatives plan flags.
    enable_empty_answer_recovery: bool = False
    # 0628 answer-retention: prepare_report appends a BBS candidate-findings
    ...
    # 0628 selective-delete compaction (user request): prune certainly-wrong
    ...
    # 0628: also dispatch the candidate-emergence rival sweep via spawn_or_assign
    # (not just add_task) so it actually runs. Default False. See ArcticswarmConfig.
    alt_task_force_dispatch: bool = False
    # 0628 anti-anchoring browsing-prompt block. Default False; qwen-gated.
    reframe_prompt: bool = False
    # 0629 anti-give-up + canonical-answer report behavior. When True: (1) the
